...
Una connessione SSH standard come `bandit26` — anche disponendo della password ottenuta con `cat /etc/bandit_pass/bandit26` — si chiuderebbe immediatamente per lo stesso motivo del livello precedente: la shell di login di `bandit26` è `/usr/bin/showtext`, che esegue `more ~/text.txt` e termina. Per lavorare come `bandit26` è necessario entrare attraverso il percorso PuTTY + vim descritto nel livello 25.
...
Oltre a `text.txt` — il file mostrato da `more` ad ogni login — è presente `bandit27-do`, un eseguibile setuid identico per funzionamento a `bandit20-do` del livello 19: consente di eseguire un comando arbitrario con i privilegi di `bandit27`. Lo si usa direttamente per leggere la password:
