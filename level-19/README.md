...
Il kernel usa l'effective UID per tutte le verifiche sui permessi — lettura di file, apertura di socket, chiamate di sistema riservate. È per questo che `./bandit20-do cat /etc/bandit_pass/bandit20` riesce: il `cat` viene eseguito con effective UID `bandit20`, che ha accesso in lettura al file.
